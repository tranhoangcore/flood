...
const client = {
  addFiles(user, services, req, callback) {
    ...
    files.forEach((file, index) => {
      ...
      fileRequest.addFiles({
        files: file, path, isBasePath, start, tags,
      });
  ...
  addUrls(user, services, data, callback) {
    ...
    request.createDirectory({path});
    request.addURLs({
      urls, path, isBasePath, start, tags,
    });
  ...
  findFilesByIndicies(indices, fileTree = {}) {
    ...
    if (directories != null) {
      selectedFiles = selectedFiles.concat(
        Object.keys(directories).reduce((accumulator, directory) => accumulator.concat(this.findFilesByIndicies(indices, directories[directory])), []),
      );
    }
  ...
  moveTorrents(user, services, data, callback) {
    ...
    const hashesToRestart = hashes.filter(hash => !services.torrentService.getTorrent(hash).status.includes(torrentStatusMap.stopped));
